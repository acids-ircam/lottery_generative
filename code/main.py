...
from model_ddsp import LotteryDDSP
from model_singae import LotterySINGAE
from model_wavenet import LotteryWavenet
from models.sing_ae.dsp import SpectralLoss
# Internal imports
...
# Data parameters
parser.add_argument("--datadir",            default="/Users/esling/Datasets/audio/",type=str,   help="Directory to find datasets")
parser.add_argument("--dataset",            default="onsets",       type=str,       help="mnist | cifar10 | fashion_mnist | cifar100 | toy")
# Model parameters
parser.add_argument("--model",              default="ddsp",         type=str,       help="mlp | gated_mlp | cnn | gated_cnn | res_cnn")
...
parser.add_argument('--type_mod',           default='cnn',          type=str,       help='')
...
parser.add_argument('--eval_interval',      default=90,             type=int,       help='')
parser.add_argument('--epochs',             default=20,             type=int,       help='')
parser.add_argument('--nbworkers',          default=2,              type=int,       help='')
...
if (args.model in ['mlp', 'gated_mlp']): 
    ...
    model = LotteryWavenet(args)
elif (args.model == "ddsp"):
    model = LotteryDDSP(args)
elif (args.model == 'sing_ae'):
    model = LotterySINGAE(args)
else:
...
if (args.model in ['ae', 'vae', 'wae', 'vae_flow']):
    criterion = nn.L1Loss()
elif (args.model in ['sing_ae']):
    criterion = SpectralLoss(nn.MSELoss(reduction='mean').to(args.device)).to(args.device)
else:
